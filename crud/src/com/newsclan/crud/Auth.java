...
public class Auth {
	...
	public static boolean isAdminOfTable(int user_id,String tbname) {
		if( user_id == 1) return true;
		if(Config.get("login.check").equalsIgnoreCase("false")) return true;
		return checkPrivilegeForRightOfTable(user_id,tbname,"admin");
	}
	...
	public static boolean canInsertTable(int user_id, String tbname) {
		...
		if (isAdmin(user_id)||isAdminOfTable(user_id,tbname))
			return true;
