...
public class VTM {
	...
	public static String getTagListOfManualTask(String tbname,int id){
		...
		for (List row : rows) {
			sb.append("<tr "+tbname+"_id='"+row.get(0)+"' "
								    + "start_time='"+row.get(2)+"' "
								    + "end_time='"+row.get(3)+"' "
								    + "tagtext='"+row.get(4)+"' x='0' y='0' w='0' h='0' "
								    + ">\n");
...
//				}
	...
	public static void importSRT(int tb_manual_task_id,String srtFile){
		String tbname=DAO.queryString("select t.table_name from tb_tag_type t inner join tb_manual_task m on t.id=m.tb_tag_type_id and m.id=?", tb_manual_task_id);
		StringBuffer sb = new StringBuffer();
		FileInputStream fis = null;
		try {
			...
			String sql="insert into "+tbname+" (tb_manual_task_id,start_time,end_time,`"+fieldname+"`) values(?,?,?,?)\n";
			
			List<String[]> list = parseSRT(sb.toString());
			for (String[] v : list) {
				
				//System.out.format(sql.replaceAll("\\?", "%s"),v[1],v[2],v[3]);
...
//					input.nextLine();
	...
	public static String build(final Long build_task_id) {
		...
		String manual_task_id = DAO.queryString("select tb_manual_task_id from tb_build_task where id=?",
				build_task_id);
		List task = DAO.queryList("select tb_movie_id,tb_tag_type_id from tb_manual_task where id=?", false, manual_task_id)
				.get(0);
		final String movie_id = String.valueOf(task.get(0));
		final String movie_path = DAO.queryString("select movie_file from tb_movie where id=?", movie_id);
		...
		if(tbname.equals("tb_av_emo_association")) fieldname="lines";
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					zipOutputStream.close();
					DAO.update("insert into tb_download_zip(update_time,report_name,zip_file) values(now(),?,?)" ,name,target.substring(Config.path.length()));
				} catch (Exception e) {
					e.printStackTrace();
				}
				try {
